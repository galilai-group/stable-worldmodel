"""Base ``Dataset`` class plus composition wrappers (Concat / Merge / Goal).

Concrete backends live under :mod:`stable_worldmodel.data.formats` and are
wired into the registry via :func:`stable_worldmodel.data.format.register_format`.
"""
...
class MergeDataset:
    """Horizontal join of multiple equal-length datasets.
...
class ConcatDataset:
    """Vertical concatenation of multiple datasets."""
...
class GoalDataset:
    """Wrap a dataset to add a sampled goal observation per item.

    Goals are sampled from one of four buckets — random / geometric future /
    uniform future / current — according to ``goal_probabilities`` (which
    must sum to 1).
    """
