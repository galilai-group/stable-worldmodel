...
def maskgit_mask(tokens, mask_token_id, mask_floor=0.5, mask_ceiling=1.0):
    ...
    p = mask_floor + (mask_ceiling - mask_floor) * torch.rand(
        B, device=tokens.device
    )
...
def joint_forward(self, batch, stage, cfg):
    ...
    input_ids, labels = maskgit_mask(
        tokens,
        dyn.mask_token_id,
        cfg.get('mask_floor', 0.5),
        cfg.get('mask_ceiling', 1.0),
    )
    ...
    loss = (
        lam_loss
        + dyn_out['loss']
        + cfg.get('act_enc_weight', 1.0) * act_enc_loss
    )
